export default async function handler(req, res) {
  ...
  const privateKey = process.env.EMAILJS_PRIVATE_KEY || process.env.EMAILJS_API_KEY;

  const missing = [];
  if (!serviceId) missing.push('EMAILJS_SERVICE_ID (or NEXT_PUBLIC_EMAILJS_SERVICE_ID)');
  if (!templateId) missing.push('EMAILJS_TEMPLATE_ID (or NEXT_PUBLIC_EMAILJS_TEMPLATE_ID)');
  if (!publicKey) {
  ...
  try {
    const sendRequest = async (withAccessToken) => fetch('https://api.emailjs.com/api/v1.0/email/send', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        user_id: publicKey,
        template_params,
        ...(withAccessToken && privateKey ? { accessToken: privateKey } : {})
      })
    });
    ...
    if (!resp.ok) {
      ...
      return res.status(resp.status).json({ error: text || `EmailJS request failed (${resp.status})` });
    }
