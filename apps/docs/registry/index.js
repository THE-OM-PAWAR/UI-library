// Simplified demo registry - just export demo metadata
// The actual code strings are stored in the JSON files
export const demoRegistry = {
    "accordion-demo": () => import("./demos/accordion-demo"),
    "accordion-multiple": () => import("./demos/accordion-multiple"),
    "alert-demo": () => import("./demos/alert-demo"),
    "alert-destructive": () => import("./demos/alert-destructive"),
    "aspect-ratio-demo": () => import("./demos/aspect-ratio-demo"),
    "aspect-ratio-gallery": () => import("./demos/aspect-ratio-gallery"),
    "aspect-ratio-portrait": () => import("./demos/aspect-ratio-portrait"),
    "aspect-ratio-square": () => import("./demos/aspect-ratio-square"),
    "aspect-ratio-ultrawide": () => import("./demos/aspect-ratio-ultrawide"),
    "aspect-ratio-widescreen": () => import("./demos/aspect-ratio-widescreen"),
    "avatar-demo": () => import("./demos/avatar-demo"),
    "avatar-group": () => import("./demos/avatar-group"),
    "badge-color": () => import("./demos/badge-color"),
    "badge-demo": () => import("./demos/badge-demo"),
    "badge-link": () => import("./demos/badge-link"),
    "badge-with-icon": () => import("./demos/badge-with-icon"),
    "badge-with-spinner": () => import("./demos/badge-with-spinner"),
    "breadcrumb-collapsible": () => import("./demos/breadcrumb-collapsible"),
    "breadcrumb-custom": () => import("./demos/breadcrumb-custom"),
    "breadcrumb-demo": () => import("./demos/breadcrumb-demo"),
    "breadcrumb-dropdown": () => import("./demos/breadcrumb-dropdown"),
    "breadcrumb-link": () => import("./demos/breadcrumb-link"),
    "button-aschild": () => import("./demos/button-aschild"),
    "button-demo": () => import("./demos/button-demo"),
    "button-destructive": () => import("./demos/button-destructive"),
    "button-ghost": () => import("./demos/button-ghost"),
    "button-lg": () => import("./demos/button-lg"),
    "button-link": () => import("./demos/button-link"),
    "button-outline": () => import("./demos/button-outline"),
    "button-secondary": () => import("./demos/button-secondary"),
    "button-sizes": () => import("./demos/button-sizes"),
    "button-sm": () => import("./demos/button-sm"),
    "button-variants": () => import("./demos/button-variants"),
    "button-with-group": () => import("./demos/button-with-group"),
    "button-with-icon": () => import("./demos/button-with-icon"),
    "button-xs": () => import("./demos/button-xs"),
    "buttongroup-demo": () => import("./demos/buttongroup-demo"),
    "buttongroup-destructive": () => import("./demos/buttongroup-destructive"),
    "buttongroup-ghost": () => import("./demos/buttongroup-ghost"),
    "buttongroup-lg": () => import("./demos/buttongroup-lg"),
    "buttongroup-outline": () => import("./demos/buttongroup-outline"),
    "buttongroup-secondary": () => import("./demos/buttongroup-secondary"),
    "buttongroup-sizes": () => import("./demos/buttongroup-sizes"),
    "buttongroup-sm": () => import("./demos/buttongroup-sm"),
    "buttongroup-variants": () => import("./demos/buttongroup-variants"),
    "buttongroup-with-dropdown": () =>
        import("./demos/buttongroup-with-dropdown"),
    "calendar-demo": () => import("./demos/calendar-demo"),
    "card-custom": () => import("./demos/card-custom"),
    "card-demo": () => import("./demos/card-demo"),
    "card-profile": () => import("./demos/card-profile"),
    "card-stats": () => import("./demos/card-stats"),
    "card-with-image": () => import("./demos/card-with-image"),
    "carousel-demo": () => import("./demos/carousel-demo"),
    "carousel-drag-free": () => import("./demos/carousel-drag-free"),
    "carousel-loop": () => import("./demos/carousel-loop"),
    "carousel-vertical": () => import("./demos/carousel-vertical"),
    "chart-demo": () => import("./demos/chart-demo"),
    "chart-line": () => import("./demos/chart-line"),
    "chart-pie": () => import("./demos/chart-pie"),
    "chart-stacked": () => import("./demos/chart-stacked"),
    "chart-horizontal": () => import("./demos/chart-horizontal"),
    "chart-area": () => import("./demos/chart-area"),
    "checkbox-demo": () => import("./demos/checkbox-demo"),
    "checkbox-destructive": () => import("./demos/checkbox-destructive"),
    "checkbox-ghost": () => import("./demos/checkbox-ghost"),
    "checkbox-lg": () => import("./demos/checkbox-lg"),
    "checkbox-outline": () => import("./demos/checkbox-outline"),
    "checkbox-secondary": () => import("./demos/checkbox-secondary"),
    "checkbox-sizes": () => import("./demos/checkbox-sizes"),
    "checkbox-sm": () => import("./demos/checkbox-sm"),
    "checkbox-variants": () => import("./demos/checkbox-variants"),
    "checkbox-with-label": () => import("./demos/checkbox-with-label"),
    "checkbox-xs": () => import("./demos/checkbox-xs"),
    "collapsible-demo": () => import("./demos/collapsible-demo"),
    "collapsible-order": () => import("./demos/collapsible-order"),
    "collapsible-product": () => import("./demos/collapsible-product"),
    "collapsible-explorer": () => import("./demos/collapsible-explorer"),
    "combobox-controlled": () => import("./demos/combobox-controlled"),
    "combobox-demo": () => import("./demos/combobox-demo"),
    "combobox-groups": () => import("./demos/combobox-groups"),
    "context-menu-checkbox": () => import("./demos/context-menu-checkbox"),
    "context-menu-demo": () => import("./demos/context-menu-demo"),
    "context-menu-radio": () => import("./demos/context-menu-radio"),
    "context-menu-sub": () => import("./demos/context-menu-sub"),
    "data-table-demo": () => import("./demos/data-table-demo"),
    "data-table-pagination": () => import("./demos/data-table-pagination"),
    "data-table-selection": () => import("./demos/data-table-selection"),
    "data-table-sorting": () => import("./demos/data-table-sorting"),
    "dialog-demo": () => import("./demos/dialog-demo"),
    "dialog-scrollable": () => import("./demos/dialog-scrollable"),
    "dialog-sizes": () => import("./demos/dialog-sizes"),
    "dialog-with-form": () => import("./demos/dialog-with-form"),
    "drawer-demo": () => import("./demos/drawer-demo"),
    "drawer-directions": () => import("./demos/drawer-directions"),
    "dropdown-menu-checkboxes": () =>
        import("./demos/dropdown-menu-checkboxes"),
    "dropdown-menu-demo": () => import("./demos/dropdown-menu-demo"),
    "dropdown-menu-radio": () => import("./demos/dropdown-menu-radio"),
    "dropdown-menu-submenu": () => import("./demos/dropdown-menu-submenu"),
    "dropdown-menu-with-icons": () =>
        import("./demos/dropdown-menu-with-icons"),
    "empty-demo": () => import("./demos/empty-demo"),
    "field-demo": () => import("./demos/field-demo"),
    "headings-code": () => import("./demos/headings-code"),
    "headings-custom-color": () => import("./demos/headings-custom-color"),
    "headings-demo": () => import("./demos/headings-demo"),
    "headings-h1": () => import("./demos/headings-h1"),
    "headings-h2": () => import("./demos/headings-h2"),
    "headings-h3": () => import("./demos/headings-h3"),
    "headings-h4": () => import("./demos/headings-h4"),
    "headings-h5": () => import("./demos/headings-h5"),
    "headings-h6": () => import("./demos/headings-h6"),
    "headings-label": () => import("./demos/headings-label"),
    "headings-paragraph": () => import("./demos/headings-paragraph"),
    "headings-variants": () => import("./demos/headings-variants"),
    "hover-card-demo": () => import("./demos/hover-card-demo"),
    "image-contain": () => import("./demos/image-contain"),
    "image-cover": () => import("./demos/image-cover"),
    "image-demo": () => import("./demos/image-demo"),
    "image-fill": () => import("./demos/image-fill"),
    "image-fit": () => import("./demos/image-fit"),
    "image-none": () => import("./demos/image-none"),
    "image-scale-down": () => import("./demos/image-scale-down"),
    "input-demo": () => import("./demos/input-demo"),
    "inputgroup-demo": () => import("./demos/inputgroup-demo"),
    "inputotp-demo": () => import("./demos/inputotp-demo"),
    "item-demo": () => import("./demos/item-demo"),
    "kbd-demo": () => import("./demos/kbd-demo"),
    "label-demo": () => import("./demos/label-demo"),
    "menubar-demo": () => import("./demos/menubar-demo"),
    "navigation-menu-demo": () => import("./demos/navigation-menu-demo"),
    "pagination-demo": () => import("./demos/pagination-demo"),
    "pagination-language": () => import("./demos/pagination-language"),
    "pagination-numbers": () => import("./demos/pagination-numbers"),
    "pagination-rows": () => import("./demos/pagination-rows"),
    "popover-alignments": () => import("./demos/popover-alignments"),
    "popover-demo": () => import("./demos/popover-demo"),
    "popover-with-form": () => import("./demos/popover-with-form"),
    "popover-with-header": () => import("./demos/popover-with-header"),
    "progress-color": () => import("./demos/progress-color"),
    "progress-demo": () => import("./demos/progress-demo"),
    "progress-height": () => import("./demos/progress-height"),
    "progress-speed": () => import("./demos/progress-speed"),
    "progress-variants": () => import("./demos/progress-variants"),
    "radio-demo": () => import("./demos/radio-demo"),
    "resizable-demo": () => import("./demos/resizable-demo"),
    "resizable-vertical": () => import("./demos/resizable-vertical"),
    "scroll-area-demo": () => import("./demos/scroll-area-demo"),
    "separator-demo": () => import("./demos/separator-demo"),
    "sheet-demo": () => import("./demos/sheet-demo"),
    "sidebar-demo": () => import("./demos/sidebar-demo"),
    "sidebar-groups": () => import("./demos/sidebar-groups"),
    "sidebar-right": () => import("./demos/sidebar-right"),
    "sidebar-with-icons": () => import("./demos/sidebar-with-icons"),
    "skeleton-demo": () => import("./demos/skeleton-demo"),
    "skeleton-card": () => import("./demos/skeleton-card"),
    "skeleton-table": () => import("./demos/skeleton-table"),
    "skeleton-profile": () => import("./demos/skeleton-profile"),
    "skeleton-list": () => import("./demos/skeleton-list"),
    "skeleton-blog": () => import("./demos/skeleton-blog"),
    "skeleton-form": () => import("./demos/skeleton-form"),
    "sonner-demo": () => import("./demos/sonner-demo"),
    "sonner-types": () => import("./demos/sonner-types"),
    "sonner-description": () => import("./demos/sonner-description"),
    "sonner-position": () => import("./demos/sonner-position"),
    "sonner-close-button": () => import("./demos/sonner-close-button"),
    "sonner-rich-colors": () => import("./demos/sonner-rich-colors"),
    "sonner-direction": () => import("./demos/sonner-direction"),
    "sonner-visible-toasts": () => import("./demos/sonner-visible-toasts"),
    "sonner-gap": () => import("./demos/sonner-gap"),
    "spinner-colors": () => import("./demos/spinner-colors"),
    "spinner-demo": () => import("./demos/spinner-demo"),
    "spinner-icons": () => import("./demos/spinner-icons"),
    "spinner-in-badge": () => import("./demos/spinner-in-badge"),
    "spinner-in-button": () => import("./demos/spinner-in-button"),
    "spinner-in-card": () => import("./demos/spinner-in-card"),
    "spinner-sizes": () => import("./demos/spinner-sizes"),
    "spinner-variants": () => import("./demos/spinner-variants"),
    "switch-demo": () => import("./demos/switch-demo"),
    "table-demo": () => import("./demos/table-demo"),
    "table-extended": () => import("./demos/table-extended"),
    "table-language": () => import("./demos/table-language"),
    "tabs-demo": () => import("./demos/tabs-demo"),
    "textarea-demo": () => import("./demos/textarea-demo"),
    "textarea-disabled": () => import("./demos/textarea-disabled"),
    "textarea-invalid": () => import("./demos/textarea-invalid"),
    "textarea-radius": () => import("./demos/textarea-radius"),
    "textarea-with-footer": () => import("./demos/textarea-with-footer"),
    "textarea-with-label": () => import("./demos/textarea-with-label"),
    "toggle-demo": () => import("./demos/toggle-demo"),
    "toggle-destructive": () => import("./demos/toggle-destructive"),
    "toggle-disabled": () => import("./demos/toggle-disabled"),
    "toggle-ghost": () => import("./demos/toggle-ghost"),
    "toggle-lg": () => import("./demos/toggle-lg"),
    "toggle-link": () => import("./demos/toggle-link"),
    "toggle-outline": () => import("./demos/toggle-outline"),
    "toggle-secondary": () => import("./demos/toggle-secondary"),
    "toggle-sizes": () => import("./demos/toggle-sizes"),
    "toggle-sm": () => import("./demos/toggle-sm"),
    "toggle-toolbar": () => import("./demos/toggle-toolbar"),
    "toggle-variants": () => import("./demos/toggle-variants"),
    "toggle-with-text": () => import("./demos/toggle-with-text"),
    "tooltip-demo": () => import("./demos/tooltip-demo"),
    "tooltip-demo-placements": () => import("./demos/tooltip-demo-placements"),
};
